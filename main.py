...
class Database:
    def __init__(self, name, file_path, delimiter, search_columns, result_columns):
        self.name = name
    ...
    def search(self, content, query):
        ...
        next(reader, None)  # Пропустить строку с заголовком
        for row in reader:
...
databases = [
    Database("gb0", "gb0.csv", ",", [0, 1, 2, 3], {0: 'ID', 1: 'Номер телефона', 2: 'Юзер', 3: 'Ник'}),
    Database("gb1", "gb1.csv", ",", [0, 1, 2, 3], {0: 'ID', 1: 'Номер телефона', 2: 'Юзер', 3: 'Ник'})
]
...
# Функция для поиска в нескольких базах данных
...
def handle_check_db_callback(call):
    ...
    if search_results:
        ...
        for result in search_results:
            report_text += (
                f"\n📋 Отчёт из базы данных «Глаз Бога»:\n"
                f"├📧 ID: id{result['ID']}\n"
                f"├📞 Телефон: {result['Номер телефона']}\n"
                f"├👤 Юзер: @{result['Юзер']}\n"
                f"└🏷 Ник: {result['Ник']}\n"
            )
            report_text += (
                f"\n💦 В слитой базе данных Telegram-бота «Глаз Бога» содержится информация о 774 тысячах пользователей. Включены данные, такие как ID пользователей, номера телефонов, имена и фамилии. База данных стала «утекшей» в июле 2021 года.\n"
            )
    else:
...
# Команды  /decipher, /encrypt
cipher = {
    'а': '᠀', 'б': '᠅', 'в': '᠆', 'г': '᠇', 'д': '᠈', 'е': '᠐', 'ё': '᠑',
    'ж': '᠒', 'з': '᠓', 'и': '᠔', 'й': '᠕', 'к': '᠖', 'л': '᠗', 'м': '᠘',
    'н': '᠙', 'о': 'ᠠ', 'п': 'ᠡ', 'р': 'ᠢ', 'с': 'ᠣ', 'т': 'ᠤ', 'у': 'ᠥ',
    'ф': 'ᠦ', 'х': 'ᠧ', 'ц': 'ᠨ', 'ч': 'ᠩ', 'ш': 'ᠪ', 'щ': 'ᠫ', 'ъ': 'ᠬ',
    'ы': 'ᠭ', 'ь': 'ᠮ', 'э': 'ᠯ', 'ю': 'ᠰ', 'я': 'ᠱ',
    'a': 'ᠲ', 'b': 'ᠳ', 'c': 'ᠴ', 'd': 'ᠵ', 'e': 'ᠶ', 'f': 'ᠷ', 'g': 'ᠸ',
    'h': 'ᠹ', 'i': 'ᠺ', 'j': 'ᠼ', 'k': 'ᠽ', 'l': 'ᠾ', 'm': 'ᠿ', 'n': 'ᡀ',
    'o': 'ᡁ', 'p': 'ᡂ', 'q': 'ᡃ', 'r': 'ᡄ', 's': 'ᡅ', 't': 'ᡆ', 'u': 'ᡇ',
    'v': 'ᡈ', 'w': 'ᡉ', 'x': 'ᡊ', 'y': 'ᡋ', 'z': 'ᡌ',
    'A': 'ᡍ', 'B': 'ᡎ', 'C': 'ᡏ', 'D': 'ᡐ', 'E': 'ᡑ', 'F': 'ᡒ', 'G': 'ᡓ',
    'H': 'ᡔ', 'I': 'ᡕ', 'J': 'ᡖ', 'K': 'ᡗ', 'L': 'ᡘ', 'M': 'ᡙ', 'N': 'ᡚ',
    'O': 'ᡛ', 'P': 'ᡜ', 'Q': 'ᡝ', 'R': 'ᡞ', 'S': 'ᡟ', 'T': 'ᡠ', 'U': 'ᡡ',
    'V': 'ᡢ', 'W': 'ᡣ', 'X': 'ᡤ', 'Y': 'ᡥ', 'Z': 'ᡦ',
    'А': 'ᡧ', 'Б': 'ᡨ', 'В': 'ᡩ', 'Г': 'ᡪ', 'Д': 'ᡫ', 'Е': 'ᡬ', 'Ё': 'ᡭ',
    'Ж': 'ᡮ', 'З': 'ᡯ', 'И': 'ᡰ', 'Й': 'ᡱ', 'К': 'ᡲ', 'Л': 'ᡳ', 'М': 'ᡴ',
    'Н': 'ᡵ', 'О': 'ᡶ', 'П': 'ᡷ', 'Р': 'ᢀ', 'С': 'ᢁ', 'Т': 'ᢂ', 'У': 'ᢃ',
    'Ф': 'ᢄ', 'Х': 'ᢅ', 'Ц': 'ᢆ', 'Ч': 'ᢇ', 'Ш': 'ᢈ', 'Щ': 'ᢉ', 'Ъ': 'ᢊ',
    'Ы': 'ᢋ', 'Ь': 'ᢌ', 'Э': 'ᢍ', 'Ю': 'ᢎ', 'Я': 'ᢏ'
    }

# Обратный шифр для дешифрования
...
def decrypt(text):
    return ''.join(reverse_cipher.get(char, char) for char in text)

@bot.message_handler(commands=['encrypt'])
def handle_encrypt(message):
    text_to_encrypt = message.text[len('/encrypt '):].strip()
    if text_to_encrypt:
        result = encrypt(text_to_encrypt)
        bot.reply_to(message, f"Зашифрованный текст: {result}")
    else:
        bot.reply_to(message, "Пожалуйста, введи слово или фразу для шифрования после команды /encrypt.")

@bot.message_handler(commands=['decipher'])
def handle_decrypt(message):
    text_to_decrypt = message.text[len('/decipher '):].strip()
    if text_to_decrypt:
        result = decrypt(text_to_decrypt)
        bot.reply_to(message, f"Расшифрованный текст: {result}")
    else:
        bot.reply_to(message, "Пожалуйста, введи слово или фразу для дешифрования после команды /decipher.")
# конец команд /decipher, /encrypt
